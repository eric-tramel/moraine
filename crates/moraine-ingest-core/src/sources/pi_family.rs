...
use super::{
    ...
    NormalizedPartials, SourceMetadata,
};
use serde_json::{json, Value};

#[derive(Debug, Clone, Copy)]
pub(super) struct PiFamilyPolicy {
    harness: &'static str,
    raw_only_fallback: bool,
}

impl PiFamilyPolicy {
    pub(super) const PI: Self = Self {
        harness: "pi-coding-agent",
        raw_only_fallback: false,
    };

    pub(super) const PRIME: Self = Self {
        harness: "prime-agent",
        raw_only_fallback: true,
    };
}

pub(super) fn pi_family_source_metadata(record: &Value) -> SourceMetadata {
...
pub(super) fn normalize_pi_family_record(
    record: &Value,
    ctx: &RecordContext<'_>,
    top_type: &str,
    base_uid: &str,
    model_hint: &str,
    policy: PiFamilyPolicy,
) -> NormalizedPartials {
    debug_assert_eq!(ctx.harness, policy.harness);
    let mut emitter = SourceEmitter::new(ctx);

    match top_type {
        "session" => handle_pi_session(record, base_uid, &mut emitter),
        "message" => handle_pi_message(record, base_uid, model_hint, policy.harness, &mut emitter),
        "model_change" => handle_pi_model_change(record, base_uid, policy.harness, &mut emitter),
        ...
        "custom" if !policy.raw_only_fallback => handle_pi_custom(record, base_uid, &mut emitter),
        "custom" => {}
...
fn handle_pi_message(
    ...
    match to_str(message.get("role")).as_str() {
        ...
        "assistant" => handle_pi_assistant_message(
            record,
            message,
            base_uid,
            model_hint,
        ...
        "toolResult" => handle_pi_tool_result_message(
            record,
            message,
            base_uid,
            model_hint,
        ...
        "bashExecution" => handle_pi_bash_execution_message(
            record,
            message,
            base_uid,
            model_hint,
        ...
        "custom" => handle_pi_custom_role_message(
            record,
            message,
            base_uid,
            model_hint,
        ...
        "branchSummary" => handle_pi_branch_summary_message(
            record,
            message,
            base_uid,
            model_hint,
        ...
        "compactionSummary" => handle_pi_compaction_summary_message(
            record,
            message,
            base_uid,
            model_hint,
...
fn handle_pi_model_change(
    ...
    let model = canonicalize_pi_family_model(model_harness, &to_str(record.get("modelId")));
...
fn canonicalize_pi_family_model(model_harness: &str, raw_model: &str) -> String {
    canonicalize_model(model_harness, raw_model)
}

fn pi_stamp_message_common(
    event: EventBuilder,
    message: &Value,
    model_hint: &str,
    model_harness: &str,
) -> EventBuilder {
    let model = canonicalize_pi_family_model(model_harness, &to_str(message.get("model")));
    let model = if model.is_empty() {
        canonicalize_pi_family_model(model_harness, model_hint)
    } else {
        model
    };

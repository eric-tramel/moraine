---
name: release
description: Cut and publish Moraine releases from a version argument such as /release X.Y.Z. Use when Codex is asked to run Moraine's release process, bump release-managed versions, create and merge the release PR, push the vX.Y.Z tag, update GitHub release notes for a general audience, verify the release-moraine workflow, and confirm the PyPI moraine-cli package. Start or continue a Codex goal for the release.
---

# Release

Run Moraine releases end to end. A `/release X.Y.Z` invocation means: publish
`vX.Y.Z`, not just prepare a plan.
...
```bash
python3 .codex/skills/release/scripts/bump-version.py "$VERSION"
```
...
Use the dev sandbox required by `AGENTS.md` when the release includes ingest,
MCP, monitor, ClickHouse schema, or monitor UI changes since the previous tag.
Capture the sandbox id with `--quiet`, do the focused checks inside it, and
tear it down before reporting completion.

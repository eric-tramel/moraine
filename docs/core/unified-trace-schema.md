...
| `endpoint_kind` | `generation` for current Codex trace rows. | `generation` for current Claude Code trace rows. |
...
| `token_usage_buckets` | Canonical additive bucket map derived from OpenAI usage details, including uncached text, cache read/write, image/audio, and reasoning buckets when present. | Canonical additive bucket map derived from Anthropic usage, including separate text input, cache read/write, output, reasoning, and `server_tool_use` counters when present. |
| `token_usage_native_units` | Native-unit companion map for image pixels, audio seconds, and image counts; current Codex traces do not emit nonzero native units. | Native-unit companion map for image pixels, audio seconds, and image counts; current Claude Code traces do not emit nonzero native units. |

...
| `harness` | Optional exact, case-sensitive normalized harness filter. Supported values are `codex`, `claude-code`, `cursor`, `hermes`, `kimi-cli`, `qwen-code`, `opencode`, and `pi-coding-agent`. |
| `source` | Optional exact, case-sensitive ingest source filter. The default configured values are `claude`, macOS-only `claude-cowork`, `codex`, `cursor`, `cursor-sqlite`, `hermes`, `kimi-cli`, `omp`, `opencode`, `pi`, and `qwen-code`; each server's MCP tool instructions list its actual configured source names. |

...
| `crates/moraine-ingest-core/tests/goldens/source_normalization/*.json` / `moraine-ingest-core` | Twelve full-shape normalized outputs complement focused assertions; only intentionally unstable fields are normalized by the test. | Check with `cargo test -p moraine-ingest-core --test golden_fixtures --locked`. |

...
| `fixtures/prime-agent/*.jsonl` / Prime Agent adapter | Sanitized Prime Agent v0.7/session-v3 root and RLM-child transcripts cover conversation, image, tool, token, raw-only orchestration, and hierarchy semantics. | `cargo test -p moraine-ingest-core --test prime_agent_fixture --locked` plus the `golden_fixtures` command above; review intentional fixture and `prime_agent.json` golden changes together. |
| `crates/moraine-ingest-core/tests/goldens/source_normalization/*.json` / `moraine-ingest-core` | Fourteen full-shape normalized outputs complement focused assertions; only intentionally unstable fields are normalized by the test. | Check with `cargo test -p moraine-ingest-core --test golden_fixtures --locked`. |

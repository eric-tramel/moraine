...
| `moraine-conversations/live_clickhouse::live_canonical_open_parity` | `scripts/dev/sandbox/run-live-test canonical-open-parity`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_parity -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Seeds one rich canonical `events` corpus (empty/blank-session, single-turn, tool-only, multi-turn, non-contiguous `turn_index` override recurrence, multi-host ordering, terminal-in-middle-turn, 035 metadata precedence, malformed-`record_ts` sentinel, and in/out trailing-slash scope), publishes source heads, and backfills BOTH read models. It then diffs the v2 canonical reader against the v1 projector field-by-field for `open(session\|turn\|event)` across full pagination, whitelisting only the three documented divergences (snapshot `None` vs `Some`, trichotomy collapse, deterministic `turn_id` within the v1 value set); the sentinel-`record_ts` fixture is asserted for v2 order stability across a same-uid re-insert (no ghost row) rather than diffed. | T3 manual/scheduled; the issue #598 WI-10 parity exit gate. Required for canonical-reader, migration-036 MV, or projector derivation changes. |
...
`list-bench`) do the same for the LIST side of the same cutover: parity against
the projected-header path it replaces, a query log that proves the retired
relations are gone, and a boundedness benchmark that makes "poll cost is flat as
unrelated event content grows" an assertion rather than a claim.

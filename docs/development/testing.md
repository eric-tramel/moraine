...
| Live query-envelope exit gates (issue #600) | `scripts/dev/sandbox/run-live-test envelope-query-log`, `scripts/dev/sandbox/run-live-test envelope-cancellation`, `scripts/dev/sandbox/run-live-test envelope-shared-budget`, or `scripts/dev/sandbox/run-live-test envelope-spill` | Fresh owned sandbox and exact ignored test per gate. |
| Live canonical `open` exit gates (issue #598) | `scripts/dev/sandbox/run-live-test canonical-open-parity`, `scripts/dev/sandbox/run-live-test canonical-open-locator`, `scripts/dev/sandbox/run-live-test canonical-open-continuation`, `scripts/dev/sandbox/run-live-test canonical-open-fence`, or `scripts/dev/sandbox/run-live-test canonical-open-bench` | Fresh owned sandbox and exact ignored test per gate. `append-to-visible` / `fsync-to-open-valid` (WI-11) are pre-wired modes whose bodies land in a later work item. |
| Live session-discovery exit gates (issue #599) | `scripts/dev/sandbox/run-live-test list-parity`, `scripts/dev/sandbox/run-live-test list-query-log`, or `scripts/dev/sandbox/run-live-test list-bench` | Fresh owned sandbox and exact ignored test per gate. Required for changes to session listing, the monitor session feed, migration-036 directory reads, or the LIST metadata fold. |
| Live bounded-search exit gates (issue #597) | `scripts/dev/sandbox/run-live-test bounded-search-exec` or `scripts/dev/sandbox/run-live-test bounded-search-attribution` | Fresh owned sandbox and exact ignored test per gate. Required for changes to `search_canonical.rs`, the shared bounded ranking CTEs, or any MCP/event/conversation search statement: a SQL-shape test cannot catch an outer reference to a column the inner derived table fails to project, which resolves only at execution time. |
...
Only these nine ignored tests are supported. Never use a blanket `--ignored` command.
...
| `moraine-conversations/live_clickhouse::live_envelope_query_log_coverage` | `scripts/dev/sandbox/run-live-test envelope-query-log`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_envelope_query_log_coverage -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Migrates and seeds the standard fixture, then drives one enveloped instance of each interactive shape (session list, bounded open, event search, MCP search, file attention, monitor-path heartbeat/table/health reads), one explicit cancel, and waits for the detached search-telemetry insert. After `SYSTEM FLUSH LOGS` it asserts every statement against the owned database carries a `moraine-*` envelope id with a finite, parseable `max_execution_time` at or under the interactive class budget; that `moraine-kill-*` and `moraine-telemetry-*` one-shots carry their own finite administrative budgets; and that zero owned-database statements ran unenveloped. | T3 manual/scheduled; the issue #600 query-log exit gate. Required for transport/envelope, repository statement-shape, or query-budget changes. |
| `moraine-conversations/live_clickhouse::live_envelope_abandoned_query_cancelled` | `scripts/dev/sandbox/run-live-test envelope-cancellation`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_envelope_abandoned_query_cancelled -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Starts a deliberately slow (~120s) statement under a 60s Interactive envelope and abandons it two ways: aborting the running task, and dropping the inner future via a 500ms `tokio::time::timeout` while the request scope completes normally (the historical #576 orphan shape). Each abandoned statement must disappear from `system.processes` within the 10-second cancellation budget (amendment A13), after provably reaching the server. A statement that completes normally must attract zero KILL statements. | T3 manual/scheduled; the issue #600 cancellation exit gate. Required for drop-guard, KILL-path, or transport-timeout changes. |
| `moraine-conversations/live_clickhouse::live_envelope_shared_budget_and_statement_cap` | `scripts/dev/sandbox/run-live-test envelope-shared-budget`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_envelope_shared_budget_and_statement_cap -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Under purpose-built budgets: a 3-statement cap admits exactly 3 server statements and refuses the fourth typed `resource_exhausted` (also proven end-to-end as `RepoError::ResourceExhausted` from a capped repository open that issues at most one statement); a 2s absolute deadline refuses the next statement client-side with nothing new in `system.query_log`; a bounded open's child statements share the `{request}-{seq}` prefix, stay under the class cap, and carry non-increasing `max_execution_time` values; and a 1,000-row cumulative allowance shrinks the second statement's `max_rows_to_read` ceiling to the remainder, fails an over-limit read typed `resource_exhausted` server-side, and refuses admission outright once drained. | T3 manual/scheduled; the issue #600 shared-budget exit gate. Required for envelope accounting, allowance, or budget-mapping changes. |
| `moraine-conversations/live_clickhouse::live_envelope_spill_and_memory_ceiling` | `scripts/dev/sandbox/run-live-test envelope-spill`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_envelope_spill_and_memory_ceiling -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Runs a synthetic ~6M-key group-by (generated by `numbers()`, sized for the sandbox) under a 256 MiB memory / 32 MiB spill Interactive budget and asserts it completes with `ExternalAggregationWritePart`/`ExternalSortWritePart` evidence and the envelope's spill threshold in `system.query_log` Settings; then repeats it with spill effectively disabled under a 128 MiB ceiling and asserts a typed `resource_exhausted` failure with server exception code 241, no orphan in `system.processes`, and a still-healthy server. | T3 manual/scheduled; the issue #600 spill/memory exit gate. Required for memory-budget, spill-threshold, or managed-profile changes. |
| `moraine-conversations/live_clickhouse::live_canonical_open_parity` | `scripts/dev/sandbox/run-live-test canonical-open-parity`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_parity -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Seeds one rich canonical `events` corpus (empty/blank-session, single-turn, tool-only, multi-turn, non-contiguous `turn_index` override recurrence, multi-host ordering, terminal-in-middle-turn, 035 metadata precedence, malformed-`record_ts` sentinel, and in/out trailing-slash scope), publishes source heads, and backfills the canonical read indexes. The v1 projector was the original field-by-field oracle; issue #603 WI-10 retired it with the projection, so the gate now proves the properties that diff certified and that still have an oracle: page-size independence (the full multi-page traversal of an arm equals its single-page read), the pinned derivation values (`turn_id`, 035 metadata precedence, sentinel ordering), and origin-scope visibility. The sentinel-`record_ts` fixture is asserted for v2 order stability across a same-uid re-insert (no ghost row). | T3 manual/scheduled; the issue #598 WI-10 parity exit gate. Required for canonical-reader, migration-036 MV, or canonical-derivation changes. |
| `moraine-conversations/live_clickhouse::live_canonical_open_locator` | `scripts/dev/sandbox/run-live-test canonical-open-locator`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_locator -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Seeds a replaced/replaying generation of one source plus an unrelated session. `open(event)` via `mcp_event_locator` returns `Ok(None)` for a replaying-only (unpublished) or superseded generation uid and resolves the live one; mid-replay reads expose complete-old until activation then complete-new, never mixed; `system.query_log` proves the locator seek referenced no unrelated session. | T3 manual/scheduled; the issue #598 WI-10 locator exit gate. Required for locator/navigation seek or non-live-generation-rejection changes. |
| `moraine-conversations/live_clickhouse::live_canonical_open_continuation` | `scripts/dev/sandbox/run-live-test canonical-open-continuation`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_continuation -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Exercises cursor continuation under concurrency: a quiescent continuation's second page issues no `mcp_event_navigation` read (directory point-read only, asserted from `system.query_log`); concurrent in-order appends continue via the anchor boundary guard with a gap-free/duplicate-free traversal versus a fresh full read; an out-of-order (older-`record_ts`) prefix insert returns the structured reopen; and an unrelated-source publication that moves the global revision does NOT reopen the target cursor (step-3 fingerprint precision). | T3 manual/scheduled; the issue #598 WI-10 continuation exit gate (design D1). Required for boundary-guard, staleness-layer, or cursor-signal changes. |
| `moraine-conversations/live_clickhouse::live_canonical_open_fence` | `scripts/dev/sandbox/run-live-test canonical-open-fence`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_fence -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Proves BINDING D9 at the read boundary: an ordinary append to a live fenced source is visible and the v2 `AnchoredSession` open proceeds with no spurious `ReadModelChanged`, while a genuine revision move (a published replacement generation) flips the session heads-fingerprint and returns the structured reopen (complete-old → complete-new, never mixed). | T3 manual/scheduled; the issue #598 WI-10 fence exit gate. Required for append-fence read-class or revision-move reopen changes. |
| `moraine-conversations/live_clickhouse::live_canonical_open_boundedness_benchmark` | `scripts/dev/sandbox/run-live-test canonical-open-bench`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_canonical_open_boundedness_benchmark -- --exact --ignored --nocapture` inside a caller-owned sandbox | Seeds ONLY through canonical `events` + publication control + the migration-036 read indexes (never `mcp_open_*`): a 50-turn session, a 1,000-event single turn, a 50k-event / ~5,000-turn monster (PR-604 profile, 10 events/turn, override-free so the early-stop path runs), an E/2E/4E trio, and a 1M-event / 100k-session unrelated corpus. Drives the v2 reader under per-phase `moraine-issue598-*` Interactive envelopes and asserts from `system.query_log`: warm session/turn/event opens ≤ 1000/750/500 ms; peak per-query `memory_usage` ≤ 512 MiB on the monster (BINDING D10); a full traversal at `max_results = 25` completes ≤ 30 s and rereads ≤ 10× one reference narrow scan (page 1's single session-wide header pass); one mid-traversal continuation page reads work bounded by the page size, not the 50k session (proving the header carry + early-stop, not a per-page full rescan); a 1M-event unrelated corpus does not grow a small open's read cost; and zero measured statement references `mcp_open_*`. Requires about 2 GB free. | T3 manual regression benchmark; the issue #598 WI-09 boundedness exit gate. The `open_v2` default flip ships only with this green on the reference host (1 CPU / 8 GiB; local plumbing runs are `authoritative:false`). Latency and bounded-cost assertions are pass conditions. |
| `moraine-conversations/live_clickhouse::live_session_list_directory_parity` | `scripts/dev/sandbox/run-live-test list-parity`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_session_list_directory_parity -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Seeds ONE `events` corpus covering omp and non-omp title/summary precedence, terminal-in-a-middle-turn against terminal-in-the-last-turn, blank and whitespace-only session ids, one session whose harness differs between two source files, one session per mode, the four `--project-only` scope shapes, and a replaceable source generation. First asserts that an unpublished store refuses `list_sessions` typed — issue #603 WI-10 removed the projected-header fallback that used to answer here, so refusing is now the contract. Then publishes readiness and, for every filter × `sort ∈ {desc, asc}` arm, diffs the full multi-page traversal of the directory path against its single-page read across all 16 `McpSessionListItem` fields; the retired projected-header path was the original oracle, and page-size independence is the property that diff was stressing. Both unfiltered traversals must equal the seeded session set, so two empty results cannot pass as agreement. Independently pins the LIST metadata fold against the frozen projector golden, the last-turn `completed` rule, latest-wins `harness`, blank-id exclusion, the §2.3 stale-`mode_hint` landmine (a session whose stored hint predates its tool name must still be returned for `mode = mcp_internal`), and the §2.7 tombstone replacement. | T3 manual/scheduled; the issue #599 §5.1 parity exit gate. Required for session-listing, LIST-fold, migration-036 directory, or readiness-refusal changes. |
| `moraine-conversations/live_clickhouse::live_session_list_query_log_clean` | `scripts/dev/sandbox/run-live-test list-query-log`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_session_list_query_log_clean -- --exact --ignored --nocapture` inside the wrapper-owned sandbox | Publishes the canonical read indexes, then issues one project-scoped MCP `list_sessions` page and one project-scoped monitor `/api/v1/sessions` page under distinguishable envelope query-id prefixes. Asserts from `system.query_log` that neither arm's statements reference `v_session_summary`, `v_conversation_trace`, `v_turn_summary`, any `mcp_open_*` relation (retired by issue #603 WI-10, so a path that reopened one would now also fail loudly on the missing table), or the corpus-wide `argMin(cwd, …)` session-origin subquery, and — positively — that both `mcp_session_directory` and `mcp_event_navigation` do appear, so an empty or mis-filtered log selection cannot pass the negative half for free. | T3 manual/scheduled; the issue #599 §5.2 read-path exit gate. Required for discovery statement-shape, scope-resolution, or readiness-gate changes. |
| `moraine-conversations/live_clickhouse::live_session_list_boundedness_benchmark` | `scripts/dev/sandbox/run-live-test list-bench`; raw: `cargo test -p moraine-conversations --test live_clickhouse --locked live_session_list_boundedness_benchmark -- --exact --ignored --nocapture` inside a caller-owned sandbox | Seeds a reference corpus, an identically shaped corpus carrying 50× the transcript text, a 40,000-session growth corpus, and a selective-filter fixture, consolidating with `OPTIMIZE … FINAL` before every measurement block. Drives pages under per-phase `moraine-issue599-*` Interactive envelopes and asserts from `system.query_log`, per phase AND per statement class: 50× transcript text moves neither total nor hydration `read_bytes` beyond 2×; `read_rows` moves by at most a constant granule allowance; MCP and monitor response payloads drift under 10 % and carry no `turns`/`text` key at any depth; the candidate pass reads at most 3× the directory's own row count (also the §2.8 follow-up-migration trigger); a one-session page's hydration stays at its granule floor over the largest corpus; a selective filter narrows the CANDIDATE set, not just the served page; ≤ 16 statements per page; peak per-query `memory_usage` ≤ 512 MiB; and warm page-1 ≤ 1000 ms. Emits an evidence JSON of every measurement, asserted or not. Requires about 2 GB free. | T3 manual regression benchmark; the issue #599 §5.3 boundedness exit gate. It did **not** gate the projected-header fallback's removal: issue #603 WI-10 deleted that fallback along with the `mcp_open_*` projection it read, and this benchmark was not run for that change. What it measures is the canonical directory path's bounded cost, which the fallback neither served nor influenced — the fallback's own exit gate was `live_session_list_directory_parity`, whose typed-refusal arm replaced it. It remains the exit gate for the directory path itself, and no §5.3 boundedness claim may be made without it green on the reference host. Latency and bounded-cost assertions are pass conditions, and the run needs a quiet host: it has failed twice from host contention alone. |
...
immediately before the durable rename and ends only after generation 2 is
visible. Content-free ingest ACK observations count actual replay flush
batches. (The legacy MCP compatibility observations this probe used to make
retired with the `mcp_open_*` projection — issue #603 WI-10 / migration 041.)
The artifact is `source-publication-replay-probe.json` and records raw batch
sequences, exact generation/checkpoint/readiness states, and
before/after/delta rows, active parts, compressed bytes, and bytes on disk for
the physical data and publication-control tables. Net `bytes_on_disk` growth
is reported per retained inactive generation; those rows are persistent and
reclamation remains #603.
...
analytics schema, monitor/repository parity, and the query-envelope exit gates
remain live semantic checks, not benchmarks:
...
scripts/dev/sandbox/run-live-test envelope-query-log
scripts/dev/sandbox/run-live-test envelope-cancellation
scripts/dev/sandbox/run-live-test envelope-shared-budget
scripts/dev/sandbox/run-live-test envelope-spill
scripts/dev/sandbox/run-live-test canonical-open-parity
scripts/dev/sandbox/run-live-test canonical-open-locator
scripts/dev/sandbox/run-live-test canonical-open-continuation
scripts/dev/sandbox/run-live-test canonical-open-fence
scripts/dev/sandbox/run-live-test canonical-open-bench
scripts/dev/sandbox/run-live-test list-parity
scripts/dev/sandbox/run-live-test list-query-log
scripts/dev/sandbox/run-live-test list-bench
```

The issue #598 canonical `open` exit gates above run one owned sandbox each.
The `append-to-visible` / `fsync-to-open-valid` (WI-11) modes are pre-wired in
`run-live-test` and its unit tests; their Rust test bodies land with that work
item, at which point the modes execute without any further edit to the
dispatcher.

The issue #599 session-discovery gates (`list-parity`, `list-query-log`,
`list-bench`) do the same for the LIST side of the same cutover: page-size
independence plus a typed refusal on an unpublished store — the projected-header
path that was `list-parity`'s original oracle retired with the projection in
issue #603 WI-10, so the refusal is now the contract (see that gate's row
above) — a query log that proves the retired relations are gone, and a
boundedness benchmark that makes "poll cost is flat as unrelated event content
grows" an assertion rather than a claim.

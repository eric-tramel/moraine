...
doctests. Normal Cargo/libtest timeouts apply unless a row says otherwise. Temporary
files, port-0 listeners, child processes, and environment changes are owned by the
invoking test and must be restored before it returns.
...
| --- | --- | --- | --- | --- |
| `moraine-conversations/repository_integration` | `integration`; repository, SQL-wire, cache, search, sessions, analytics | `cargo test -p moraine-conversations --test repository_integration --locked` | Rust/Cargo. One executable uses owned Axum mock-ClickHouse listeners on `127.0.0.1:0`; Tokio owns task/socket teardown; no real ClickHouse or persistent files. | T0; normal libtest timeout. The 67-test move map is `crates/moraine-conversations/tests/repository_integration/test-name-map.json`. |
...
| --- | --- | --- | --- |
| `moraine-conversations/analytics_latency` | Compile: `cargo bench -p moraine-conversations --bench analytics_latency --locked --no-run`. Smoke: `MORAINE_BENCH_MONITOR_URL=<owned-monitor-url> MORAINE_BENCH_CLICKHOUSE_URL=<owned-clickhouse-url> MORAINE_BENCH_DATASET_MANIFEST=<owned-dataset-manifest.json> cargo bench -p moraine-conversations --bench analytics_latency --locked -- --profile smoke --output target/bench/analytics-latency-smoke.json`; full substitutes `--profile full` and `...-full.json`. The seed owner writes the verified manifest as `{"fingerprint":"sha256:<64-lowercase-hex>","cardinality":<seeded-row-count>}`; optional database/user/password variables identify that same owned dataset for both arms. Helpers: `cargo test -p moraine-conversations --test analytics_benchmark_support --locked`. | Caller provisions one owned live stack/database, derives the manifest from the complete canonical seeded corpus, and always tears it down (for a sandbox: `scripts/dev/sandbox/moraine-sandbox down <id>`). The bench verifies the manifest before measurement and verifies the corpus stayed unchanged afterward. It is read-only, uses 30s HTTP requests, and owns only its atomic output; unlike the live-test wrapper, it does not provision or clean the stack. Custom Cargo bench, not Criterion. | T4 compile/deterministic on PR when wired; reduced live smoke/manual and full manual. Missing URLs/manifest, corpus drift, semantic/cardinality/digest/output failure is nonzero; timing non-blocking. |
...
| `replay_mcp_latency` / persistent and cold-process | `python3 scripts/bench/replay_mcp_latency.py --config <moraine.toml> --profile smoke --mode persistent --output-json target/bench/replay-mcp-persistent-smoke.json`; use `cold_process` for the distinct lifecycle boundary and `full` for full runs. Check: `python3 -m unittest scripts/bench/test_replay_mcp_latency.py`. | Populated telemetry, production `bin/moraine`/`moraine-mcp`, exact git provenance. Persistent owns one long-lived MCP child; cold owns one per sample. Per-RPC timeout and TERM/KILL cleanup are producer-owned. | T4; modes are fingerprint-distinct and never compared to one another. Semantic/output/child/timeout failure is nonzero; timing non-blocking. |
| `mcp_two_tool_sla` / MCP tool matrix | `python3 scripts/bench/mcp_two_tool_sla.py --config <owned-config> --profile smoke --output-json target/bench/mcp-two-tool-sla.json` (or `--profile full`; full defaults warmup=1/repeats=5/min-docs=100000). Check: `python3 scripts/bench/test_mcp_two_tool_sla.py`. | Python 3, read-only ClickHouse corpus, production binaries, exact git provenance. One owned stdio MCP child; 20s RPC timeout; close stdin, TERM/wait 5s, KILL/wait 5s. | T4. Missing corpus/prerequisite, tool/oracle/insufficient sample, validation/write failure is nonzero. Historical SLA values are diagnostics; v1 timing stays `not_evaluated`. |
...
python3 <issue-477-checkout>/docs/development/test-architecture/generate_baseline.py \
  ...
  --output <issue-477-checkout>/docs/development/test-architecture/baseline-77c90d6
```

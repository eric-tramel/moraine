...
```bash
moraine setup --yes --mcp-target claude-code --mcp-target codex --mcp-target hermes --mcp-target kiro-cli --mcp-target kimi-cli --mcp-target opencode --mcp-target cursor --mcp-target pi-coding-agent
```

Preview targeted MCP/plugin changes without touching host agent config:

```bash
moraine setup --dry-run --mcp-target claude-code --mcp-target codex --mcp-target hermes --mcp-target kiro-cli --mcp-target kimi-cli --mcp-target opencode --mcp-target cursor --mcp-target pi-coding-agent
```
...
directly or writes global MCP config for supported harnesses such as Kimi CLI,
OpenCode, Cursor, and Pi Coding Agent. These user-scoped integrations can search
the host-wide Moraine history visible to your user, so enable them only in
trusted harness environments.

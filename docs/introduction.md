...
harnesses such as Codex, Claude Code, Cursor, Kiro CLI, Kimi CLI, Hermes, and Pi
Coding Agent. The default setup runs on your machine and writes runtime state
under `~/.moraine`.
...
- **Realtime local ingest.** Moraine watches Codex, Claude Code, Cursor, Kiro
  CLI, Kimi CLI, Hermes, and Pi Coding Agent session files and backfills
